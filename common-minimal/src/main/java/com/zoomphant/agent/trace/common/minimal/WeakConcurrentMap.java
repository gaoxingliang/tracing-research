...
/**
 * This borrows heavily from Rafael Winterhalter's {@code com.blogspot.mydailyjava.weaklockfree
 * .WeakConcurrentMap}
 ...
 */
public class WeakConcurrentMap<K, V> extends ReferenceQueue<K> implements Iterable<Map.Entry<K,
        V>> {
    ...
    // This comment was directly verbatim from https://github
    // .com/raphw/weak-lock-free/blob/dcbd2fa0d30571bb3ed187a42cb75323a5569d5b/src/main/java/com
    // /blogspot/mydailyjava/weaklockfree/WeakConcurrentMap.java#L273-L302
    /*
     * Why this works:
     * ---------------
     *
     * Note that this map only supports reference equality for keys and uses system hash codes.
     * Also, for the
     * WeakKey instances to function correctly, we are voluntarily breaking the Java API contract
     *  for
     * hashCode/equals of these instances.
     *
     *
     * System hash codes are immutable and can therefore be computed prematurely and are stored
     * explicitly
     * within the WeakKey instances. This way, we always know the correct hash code of a key and
     * always
     * end up in the correct bucket of our target map. This remains true even after the weakly
     * referenced
     * key is collected.
     *
     * If we are looking up the value of the current key via WeakConcurrentMap::get or any other
     * public
     * API method, we know that any value associated with this key must still be in the map as
     * the mere
     * existence of this key makes it ineligible for garbage collection. Therefore, looking up a
     * value
     * using another WeakKey wrapper guarantees a correct result.
     *
     * If we are looking up the map entry of a WeakKey after polling it from the reference queue,
     *  we know
     * that the actual key was already collected and calling WeakKey::get returns null for both
     * the polled
     * instance and the instance within the map. Since we explicitly stored the identity hash
     * code for the
     * referenced value, it is however trivial to identify the correct bucket. From this bucket,
     * the first
     * weak key with a null reference is removed. Due to hash collision, we do not know if this
     * entry
     * represents the weak key. However, we do know that the reference queue polls at least as
     * many weak
     * keys as there are stale map entries within the target map. If no key is ever removed from
     * the map
     * explicitly, the reference queue eventually polls exactly as many weak keys as there are
     * stale entries.
     *
    ...
    static final class WeakKey<T> extends WeakReference<T> {
        ...
        /**
         * While a lookup key will invoke equals against this, the visa versa is not true. This
         * method
         * is only used inside the target map to resolve hash code collisions.
         */
        @Override
        public boolean equals(Object o) { // resolves hash code collisions
    ...
    public String toString() {
        ...
        expungeStaleEntries(); // Clean up so that only present references show up (unless race
      // lost)
